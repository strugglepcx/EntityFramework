...
namespace Microsoft.Data.Entity.Metadata.Internal
{
    public class InternalEntityBuilder : InternalMetadataItemBuilder<EntityType>
    {
        ...
        private bool RemoveProperty(Property property, ConfigurationSource configurationSource)
        {
            ...
            var removedRelationships = new List<RelationshipSnapshot>();
            foreach (var foreignKey in Metadata.ForeignKeys.Where(i => i.Properties.Contains(property)).ToList())
            {
                removedRelationships.Add(DetachRelationship(foreignKey, configurationSource));
            }
            ...
            foreach (var removedRelationship in removedRelationships)
            {
        ...
        {
            var navToPrincipalName = foreignKey.GetNavigationToPrincipal()?.Name;
            var navToDependentName = foreignKey.GetNavigationToDependent()?.Name;
            ...
            return new RelationshipSnapshot(relationship, navToPrincipalName, navToDependentName, relationshipConfigurationSource.Value);
        }
